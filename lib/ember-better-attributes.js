...
function generateAttribute(name, value) {
  ...
  if (binding) {
    value = "{{bind-attribute '" + name + "' " + binding[2];
    if (binding[1]) { value += " prefix='" + escapeQuotes(binding[1]) + "'"; }
    if (binding[3]) { value += " suffix='" + escapeQuotes(binding[3]) + "'"; }
    value += "}}";
    return name + '="' + value + '"';
  } else if (/\{\{/.test(value)) {
    return name + '="' + value + '"';
...
var originalCompile = Ember.Handlebars.compile;

Ember.Handlebars.compile = function(string) {
  var tokens = HTML5Tokenizer.tokenize(string);
  var template = HTML5Tokenizer.generate(tokens);
  return originalCompile(template);
};

Ember.Handlebars.originalCompile = originalCompile;
